    ...
    def transport_thread(self):
        while self.running:
            if self.start_transport:
                ...
                self.get_logger().info(f'[pick] shape={shape}, raw position={position}, yaw={yaw}')
                if position[0] > 0.22:
                    position[2] += 0.01
                config_data = common.get_yaml_data(os.path.join(self.config_path, self.calibration_file))
                position = calibrated_pose.apply_axis_calibration(position, config_data, 'kinematics').tolist()
                self.get_logger().info(f'[pick] calibrated position={[round(p,4) for p in position]}')
                if "sphere" in shape:
                ...
                if finish:
                    if "sphere" in shape:
                        ...
                        self.publish_arm(97, -118, 10, -92.5, -10, 0, 1500)
                        time.sleep(2.0)
                        self.publish_arm(97, -118, 10, -92.5, -10, -60, 1500)
                        time.sleep(1.5)
                        self.publish_arm(97, -118, 100, -92.5, -16, -60, 1500)
                        time.sleep(1.5)
                    if "cylinder" in shape:
                        # self.controller.run_action("target_2")
                        self.publish_arm(97, -118, 10, -92.5, -10, 0, 1500)
                        time.sleep(2.0)
                        self.publish_arm(97, -118, 10, -92.5, -16, -60, 1500)
                        time.sleep(1.5)
                        self.publish_arm(97, -118, 100, -92.5, -16, -60, 1500)
                        time.sleep(1.5)
                    if "cuboid" in shape:
    ...
    def shape_recognition(self, bgr_image, depth_image, depth_color_map, depth_intrinsic_matrix, min_dist):
        ...
        if min_dist_m <= 0.35:
            ...
            for obj in contours:
                ...
                # 方法1: 参考 jetarm — mask 二值图的标准差（反映轮廓圆度/填充率）
                ...
                log_msg_details = f"depth_std={depth_std:.1f}, height_std={height_std:.6f}, CornerNum={CornerNum}, aspect={aspect_ratio:.2f}"

                # 形状判断逻辑：
                # 球体：角点多(>=5) + 宽高比接近1 + 表面有弧度或轮廓近圆
                # 圆柱竖立：角点多(>=5) + 宽高比接近1 + 表面平坦
                # 圆柱躺倒：角点多 + 宽高比>1.4（椭圆形）
                # 长方体：角点少(<=4)
                
                circularity = (4 * np.pi * area) / (perimeter * perimeter) if perimeter > 0 else 0
                log_msg_details += f", circularity={circularity:.3f}"

                if CornerNum >= 5 and aspect_ratio < 1.4:
                    # 近圆形轮廓：区分球体和竖立圆柱
                    if height_std > 0.0015 or circularity > 0.85:
                        # 表面有弧度 或 轮廓非常圆 → 球体
                        sphere_index += 1
                        ...
                        angle = 0
                        long_edge_angle = 0
                elif CornerNum >= 5 and aspect_ratio >= 1.4:
                    # 椭圆形轮廓 → 躺倒圆柱
                    cylinder_index += 1
                    objType = "cylinder_" + str(cylinder_index)
                    angle = 0
                ...
                if objType is not None:
                    ...
                    if yaw is not None:
                        ...
                        config_data = common.get_yaml_data(os.path.join(self.config_path, self.calibration_file))
                        position = calibrated_pose.apply_axis_calibration(position, config_data, 'depth').tolist()
