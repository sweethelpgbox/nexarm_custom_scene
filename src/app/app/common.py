...
class Heart:
    def __init__(self, node, srv_name, timeout, callback):
        ...
        self.node = node
        self._destroyed = False  # Add destroyed flag (添加销毁标志)
    ...
    def destroy(self):
        """
        Completely destroy the Heart instance and ensure all resources are properly released (完全销毁 Heart 实例，确保所有资源被正确释放)
        """
        if self._destroyed:
            return
        
        try:
            # Stop heartbeat check (停止心跳检测)
            self.heartbeat_stamp = 0
