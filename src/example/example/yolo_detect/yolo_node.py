...
class YoloNode(Node):
    def __init__(self, name):
        ...
        self.start_service = self.get_string_param('start_service', '/yolo/start')
        self.stop_service = self.get_string_param('stop_service', '/yolo/stop')
        ...
        model_path = self.mode_path + "/" + engine + '.engine'
        self.get_logger().info('\033[1;32mmodel path: [%s]\033[0m' % model_path)
        if not os.path.exists(model_path):
            self.get_logger().error('\033[1;31mmodel file not found: [%s]\033[0m' % model_path)
        self.get_logger().info('\033[1;32mimage topic: [%s]\033[0m' % self.image_topic)
        self.yolo_detect = YOLO(model_path, task=self.task, verbose=False)

        self.colors = Colors()
        self.create_service(Trigger, self.start_service, self.start_srv_callback)  # 进入玩法(enter the game)
        self.create_service(Trigger, self.stop_service, self.stop_srv_callback)  # 退出玩法(exit the game)

        self.image_sub = self.create_subscription(Image, self.image_topic, self.image_callback, 1)  # 摄像头订阅(subscribe to the camera)
