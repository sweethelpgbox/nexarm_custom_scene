...
def calculate_e_distance(point1, point2):
    if isinstance(point1, (list, tuple, np.ndarray)) and len(point1) >= 2:
        x1, y1 = point1[0], point1[1]
    else:
        x1, y1 = point1, point1  # 默认用相同值
        
    if isinstance(point2, (list, tuple, np.ndarray)) and len(point2) >= 2:
        x2, y2 = point2[0], point2[1]
    else:
        x2, y2 = point2, point2
        
    e_distance = int(round(math.sqrt(pow(x1 - x2, 2) + pow(y1 - y2, 2))))
    return e_distance
