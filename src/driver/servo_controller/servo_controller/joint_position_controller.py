...
class JointPositionController():
    def __init__(self, joint_config, joint_name):
        ...
        # self.RADIANS_PER_ENCODER_TICK = 240 / 360 * (math.pi * 2) / 4096  # 脉宽--->弧度
        # self.ENCODER_TICKS_PER_RADIAN = 3200.0   # 每弧度对应 3200 个脉冲
        # self.RADIANS_PER_ENCODER_TICK = 1.0 / self.ENCODER_TICKS_PER_RADIAN # 每个脉冲对应的弧度 (约0.0003125)
        if 'ticks_per_rad' in joint_config:
        # 从 YAML 读取
            self.ENCODER_TICKS_PER_RADIAN = float(joint_config['ticks_per_rad'].value)
        else:
            # 兜底默认值
            self.ENCODER_TICKS_PER_RADIAN = 3200.0 
        
        # 自动计算倒数（脉宽转弧度用）
        self.RADIANS_PER_ENCODER_TICK = 1.0 / self.ENCODER_TICKS_PER_RADIAN# self.RADIANS_PER_ENCODER_TICK = 3200  # 脉宽--->弧度
        # self.ENCODER_TICKS_PER_RADIAN = 1.0 / self.RADIANS_PER_ENCODER_TICK # 弧度-->脉宽
