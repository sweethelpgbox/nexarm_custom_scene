...
def get_area_max_contour(contours, threshold=50):
    ...
    for c in contours:  # Iterate through all contours (历遍所有轮廓)
        contour_area_temp = math.fabs(cv2.contourArea(c))  # Calculate the contour area (计算轮廓面积)
        if contour_area_temp > contour_area_max:
            contour_area_max = contour_area_temp
            if contour_area_temp > threshold:  # Filter out interference (过滤干扰)
                area_max_contour = c
...
def world_to_pixels(world_points, K, T):
    ...
    Args:
        ...
        T: Extrinsic matrix [R|t] (外参矩阵 [R|t])
    Returns:
        pixel_points: List of pixel coordinates (像素坐标点列表)
    """
    pixel_points = []
    for wp in world_points:
        # Convert world coordinates to homogeneous coordinates (将世界坐标转换为齐次坐标)
        world_homo = np.append(wp, 1).reshape(4, 1)
        # Transform to camera coordinate system using the extrinsic matrix (通过外参矩阵转换到相机坐标系)
        camera_point = np.dot(T, world_homo)
        # Project onto the pixel plane (投影到像素平面)
...
def calculate_pixel_length(world_length, K, T):
    """
    Calculate the corresponding length in pixel coordinates for a given length in world coordinates (计算世界坐标中的长度在像素坐标中的对应长度)
    Args:
        world_length: Length in world coordinates (世界坐标中的长度)
        K: Camera intrinsic matrix (相机内参矩阵)
        T: Extrinsic matrix (外参矩阵)
    Returns:
        pixel_length: Length in pixel coordinates (像素坐标中的长度)
    """
    # Define start point and direction (定义起始点和方向)
    start_point = np.array([0, 0, 0])  # Start point (起始点)
    direction = np.array([0, 1, 0])  # Direction along the y-axis (y方向)

    # Calculate end point (计算终点坐标)
    end_point = start_point + direction * world_length
    # Convert both points to pixel coordinates (转换两个端点到像素坐标)
    pixels = world_to_pixels([start_point, end_point], K, T)
    # Calculate the pixel distance (计算像素距离)
